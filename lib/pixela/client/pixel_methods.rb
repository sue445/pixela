module Pixela::Client::PixelMethods
  ...
  # Subtract quantity from a specific "Pixel"
  #
  # @param graph_id [String]
  # @param date [Date,Time]
  # @param quantity [Integer,Float]
  ...
  # @example
  #   client.add_specific_pixel(graph_id: "test-graph", date: Date.today, quantity: "1")
  def subtract_specific_pixel(graph_id:, date:, quantity:)
    params = {
      quantity: quantity.to_s,
    }

    with_error_handling do
      connection.put("users/#{username}/graphs/#{graph_id}/#{to_ymd(date)}/subtract", params.compact).body
    end
  end
end
